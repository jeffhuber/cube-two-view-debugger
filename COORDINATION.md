...
- **2026-05-28** — **Cursor Cloud is a third agent active in both repos (docs-only by convention).** `AGENTS.md` (Cursor's `CLAUDE.md`-equivalent) landed at the root of both repos: cube-snap#220 + ctvd#382. Cursor Cloud agents author `AGENTS.md` and Cursor-specific dev-env onboarding; they do not touch `CLAUDE.md` or `COORDINATION.md` (those remain Claude/Codex territory). If Cursor agents start opening PRs that touch real code, we'll need to revisit the lane definitions above — for now the convention is "Cursor reads `AGENTS.md`, Claude/Codex read `CLAUDE.md`, no overlap collision." Mirror invariant doesn't apply to `AGENTS.md` itself (each repo's `AGENTS.md` is repo-specific).
