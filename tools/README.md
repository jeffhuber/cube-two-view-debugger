...
| **`fetch_ios_repro_upload.py`** | **Remote iOS repro upload fetcher.** Given an upload id, `/runs/...` path, or full upload URL, downloads the remote `bundle.json` from `api.cubesnap.app` and imports it into the same local corpus layout as `ios_repro_bundle.py`, ready for `evaluate_ios_repro_bundles.py`. |
