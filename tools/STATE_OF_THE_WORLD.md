...
| **2 — Trust policy diagnostics** | In progress. Phase 2A (#228): solo phase_sep maxes at 45.8% recall / 9.2% FPR — below bar. Phase 2B initial matrix: 17 candidate rules over existing signals (phase_sep + cv-local) — none meet the bar; closest is `phase_and_cv_T8.0` at 54.2% recall / 28.9% FPR. See `PHASE_2B_TRUST_SIGNAL_MATRIX.md`. **Next step:** extend matrix via `tools/phase2b_trust_matrix.py --recompute-global-model` to fold in fit_residual, vertex-ensemble disagreement, and two-view consistency before triggering the conditional pivot to learned geometry / capture UX. | Product-shaped: catches catastrophic / phase-wrong cases with low false-retake on GOOD cases on the 58-case eval. Concretely: ≥80% recall on the catastrophic band at ≤10% false-retake on GOOD. (Aggregate-correlation thresholds like "r > 0.6" are an internal sanity check, not the success bar.) |
