...
| `tests/fixtures/main_solvable_baseline.json` | **Committed solvable-rate snapshot** of current-main production recognizer on the corpus + hard-case manifests. Tracks per-sticker accuracy, exact matches, legal states, confident solves, and confident-wrong rows. | 30 rows (29 scored, 1 skipped) | aggregate + per-row metrics | `tools/main_solvable_baseline.py`; Phase 2/3 production guardrail baseline |
| `tests/fixtures/phase2b_trust_signal_matrix.json` | **Phase 2B trust-signal matrix.** Per-run join of phase_sep (from `post_218_baseline.json`) + cv-local structural status (from `cv_local_baseline.json`) + outcome category, plus candidate-rule evaluation against the Phase 2 bar (≥80% catastrophic recall at ≤10% GOOD false-retake). **Headline: no rule over existing signals meets the bar.** | 116 rows / 58 cases / 17 rules | per-run signals + per-rule eval | `tools/phase2b_trust_matrix.py` + `PHASE_2B_TRUST_SIGNAL_MATRIX.md` |
...
| **`tools/phase2b_trust_matrix.py`** | **Phase 2B trust-signal matrix.** Joins existing per-run (phase_sep) + per-case (cv-local status) signals into a per-run matrix, evaluates 17 candidate trust rules against the Phase 2 bar. Diagnostics-only — no production behavior change. | `tests/fixtures/phase2b_trust_signal_matrix.json` + `PHASE_2B_TRUST_SIGNAL_MATRIX.md` | **Verdict:** no rule over existing signals (phase_sep alone, cv-local alone, OR/AND compounds) meets the bar. `--recompute-global-model` flag reserved for future extension (fit_residual, vertex disagreement, two-view consistency). |
