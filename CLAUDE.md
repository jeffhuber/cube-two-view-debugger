...
meaning the EXIF-corrected natural image size
reported by the browser, not raw sideways iPhone storage pixels.
...
Prefer starting at the topmost or upper-left visible corner, then
continue clockwise or counter-clockwise. For cube hulls, click only the
outer cube silhouette in perimeter order; the normal three-face
isometric photos usually have a six-point hull. Do not include interior
face seams as hull vertices.

When multiple draft labels exist, never assume "latest timestamp" is
canonical unless the user says so. Pass explicit label JSON paths to
`tools/evaluate_geometry_labels.py` so the run is reproducible.
...
Before using those labels for cube-isolation conclusions, evaluate
both with `tools/evaluate_geometry_labels.py` and inspect the generated
overlays or JSON metrics. The evaluator reports both all assigned
`selectedGridCells` and the filtered `topVisibleTripleGridCells`; use
the latter when you need to distinguish artifact side grids from the
highest-ranked visible-face triple. Keep the comparative-claim rule
active: check both A and B metrics before claiming a Set 46 pair-level
pattern.
