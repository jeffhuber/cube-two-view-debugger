...
| **2 — Trust policy diagnostics** | In progress. Sub-status: **2A done** (#228 — solo phase_sep ceiling 45.8% recall / 9.2% FPR, below bar); **2B initial matrix done** (#232 — joined phase_sep + cv-local across 18 candidate rules using the correct `\|phase_sep\| < T` predicate; none meet the bar; closest is `phase_sep_alone_T20.0` at 66.7% recall / 30.3% FPR, see `PHASE_2B_TRUST_SIGNAL_MATRIX.md`); **2B extension pending** — extend via `tools/phase2b_trust_matrix.py --recompute-global-model` to fold in fit_residual, vertex-ensemble disagreement, and two-view consistency before triggering the conditional pivot to learned geometry / capture UX. | Product-shaped: catches catastrophic / phase-wrong cases with low false-retake on GOOD cases on the 58-case eval. Concretely: ≥80% recall on the catastrophic band at ≤10% false-retake on GOOD. (Aggregate-correlation thresholds like "r > 0.6" are an internal sanity check, not the success bar.) |
