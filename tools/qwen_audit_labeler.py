#!/usr/bin/env python3
"""Apply Qwen audit result labels from Qwen-bot PR comments.

Parallels `devin_audit_labeler.py` for the informational calibration
phase. Same label state-machine shape, different label prefix and bot
author. The Qwen audit currently does NOT authorize Claude's standing
in-thread merge delegation — see `tools/QWEN_AUDIT_PROTOCOL.md`.
...
Trailer protocol (machine-readable, authoritative over prose):
    ...
    <!-- QWEN_AUDIT_STATE: needs-qwen-audit -->   (head changed during review)
"""
...
NEEDS_LABEL = "needs-qwen-audit"
DONE_LABEL = "qwen-audit-done"
BLOCKED_LABEL = "qwen-audit-blocked"

# Author login(s) that the Qwen bridge posts audit comments under.
# Override via QWEN_BOT_AUTHORS env var (comma-separated) so users can
# point at their own service account without forking the labeler.
_DEFAULT_AUTHORS = "qwen-audit-bot,qwen-audit-bot[bot]"
QWEN_COMMENT_AUTHORS = {
    a.strip().lower()
    for a in os.environ.get("QWEN_BOT_AUTHORS", _DEFAULT_AUTHORS).split(",")
    if a.strip()
}
...
def is_qwen_comment_author(login: str) -> bool:
    return login.lower() in QWEN_COMMENT_AUTHORS
...
TRAILER_PATTERN = re.compile(
    r"<!--\s*QWEN_AUDIT_STATE:\s*"
    r"(qwen-audit-done|qwen-audit-blocked|needs-qwen-audit)"
    r"\s*-->",
    flags=re.IGNORECASE,
)

# Tightened to the structured form Qwen is instructed to emit:
#   "HEAD_CHANGED_DURING_REVIEW: reviewed <old>, current <new>"
# A bare substring match would false-positive on review prose that
# mentions the identifier (e.g., if Qwen audits this very script).
...
def classify_audit_comment(body: str) -> Optional[str]:
    ...
    if trailer:
        ...
        if label == "qwen-audit-done":
            return "done"
        if label == "qwen-audit-blocked":
            return "blocked"
        return "needs"

    # Structured HEAD_CHANGED form when no trailer — conservative requeue.
    if HEAD_CHANGED_PATTERN.search(body):
        return "needs"

    # Prose fallbacks (less reliable; only used if trailer was missed).
    if re.search(r"Qwen Audit(?:\s+Result)?\s*[—–:-]\s*PASS\b", body, flags=re.IGNORECASE):
        return "done"
    if re.search(
        (
            r"Qwen Audit(?:\s+Result)?\s*[—–:-]\s*"
...
def resolve_label_decision(
    ...
    *,
    ...
    if not is_qwen_comment_author(author):
        return None, f"ignored comment author: {author}"

    body = comment.get("body") or ""
    status = classify_audit_comment(body)
    if status is None:
        return None, "comment is not a final Qwen audit result"
